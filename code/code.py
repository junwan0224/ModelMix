...
print("Use Pruning", use_prune, "Use Mix", use_mix, "Use Norm", use_norm, "Use Trunc", use_trunc, "CLIP NORM", max_val,
...
parser.add_argument('--resume', default='save_temp/checkpointNew.th', type=str, metavar='PATH', help ='path to latest checkpoint (default: none)')
...
def main():
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[80, 110],
                                                        last_epoch=args.start_epoch - 1)
    lr_scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer2, milestones=[80, 110],
                                                         last_epoch=args.start_epoch - 1)
    lr_scheduler3 = torch.optim.lr_scheduler.MultiStepLR(optimizer3, milestones=[80, 110],
                                                         last_epoch=args.start_epoch - 1)
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if use_trunc:
            for name, param in temp_model.named_parameters():
                if "bn" not in name:
                    param.grad_sample = torch.clamp(param.grad_sample, min = -max_val, max = max_val)
        ...
        if use_prune:
            ...
            for name, param in temp_model.named_parameters():
                ...
                sum_grad = torch.squeeze(torch.sum(param.grad_sample, 0)) / batch_select
                sum_grad += noise1
                all_param = torch.cat((all_param, torch.flatten(sum_grad)), 0)
        ...
        if use_mix:
            ...
            for name, param in temp_model.named_parameters():
                ...
                if (torch.min(gap_opr) < tau):
                    ...
                    temp_dict[name] = torch.add(temp_dict[name], gap_opr * sign_opr / 2)
                    temp_dict2[name] = torch.add(temp_dict2[name], -gap_opr * sign_opr / 2)
        ...
        if use_norm:
            for name, param in temp_model.named_parameters():
                if keep_bn and "bn" in name:
                    continue
                norms = param.grad_sample.view(len(param.grad_sample), -1).norm(2, -1)
                norms += torch.add(0.0000001, norms)
                clip_factor = torch.ones(norms.shape).cuda(device) / norms
                clip_factor = clip_factor.clamp(max = 1.0)
                param.grad = torch.einsum("i,i...", clip_factor, param.grad_sample)
        else:
            for name, param in temp_model.named_parameters():
                if keep_bn and "bn" in name:
                    continue
                param.grad = torch.squeeze(torch.sum(param.grad_sample, 0)) / batch_select
