...
batch_select = 1
public_batch_size = 10
expand_batch_size = 200
calc_bn_size = 50
true_batch = 1200 / batch_select
prune_percentage = 0
...
def mixup_public_data(private_x, private_y, public_x, public_y, alpha = 0.1):
    ...
    x1, y1 = private_x.expand(x2.size()), private_y.expand(y2.size())
...
def train(train_loader, model, model2, store_model, criterion, 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if use_expand:
            ...
            inputs, y1, y2, lam = mixup_public_data(input_var, target_var, public_input, public_target)
            outputs = temp_model(inputs)
