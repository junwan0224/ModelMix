...
if use_norm:
    print("Use Norm", use_norm, "CLIP NORM", clip_norm, "Use Mix", use_mix, gap_0, "Noise", noise_mul, "Num of Epochs", num_epoch, "Device", dev)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, model2, store_model,
              ...
              epoch, prune_percentage, gap_rate)
        lr_scheduler.step()
        lr_scheduler2.step()
        lr_scheduler3.step()
...
def calc_persample_norm_list(grad_sample_list):
    sum_norms = torch.zeros(small_batch_num).cuda(device)
...
def train(train_loader, model, model2, store_model, criterion,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for name, param in temp_model.named_parameters():
            ...
            #grad_sample_list[name] = param.grad_sample.sum(0).unsqueeze(0) / small_batch

        calc_bn_num = int(batch_select / calc_bn_size)
        temp_dict = copy.deepcopy(temp_model.state_dict())
        store_model.load_state_dict(temp_dict)
        bn_store_norm = {}
        for j in range(calc_bn_num):
            new_input = input_var[j * calc_bn_size : (j + 1) * calc_bn_size]
            new_target = target_var[j * calc_bn_size : (j + 1) * calc_bn_size].long()
        ...
        if use_norm:
            ...
            for name, param in temp_model.named_parameters():
                if "bn" not in name:
                    param.grad = torch.einsum("i,i...", clip_factor, grad_sample_list[name]) / small_batch_num
                else:
            ...
            for name, param in temp_model.named_parameters():
                if "bn" not in name:
                    param.grad = grad_sample_list[name].sum(0).squeeze() / small_batch_num
                else:
                    param.require_grad = True
                    param.grad = bn_store_norm[name]
