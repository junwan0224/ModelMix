...
#parser.add_argument('--eps', default=48.32, type=float, help='privacy parameter epsilon')
#parser.add_argument('--eps', default=30.263, type=float, help='privacy parameter epsilon')
parser.add_argument('--eps', default=700, type=float, help='privacy parameter epsilon')
parser.add_argument('--delta', default=1e-5, type=float, help='desired delta')

parser.add_argument('--rgp', default=True, action='store_true', help='use residual gradient perturbation or not')
...
parser.add_argument('--aux_data_size', default=5000, type=int, help='size of the auxiliary dataset')

#batch_select = 50
#total_batch = 2500
#batch_num = total_batch / batch_select
...
if(args.dataset == 'svhn'):  ## For SVHN, we concatenate training samples and extra samples to build the training set.
    trainloader, extraloader, testloader, n_training, n_test = get_data_loader('svhn', batchsize = total_batch)
    ...
    trainloader, testloader, n_training, n_test = get_data_loader('cifar10', batchsize = total_batch)
...
sampling_prob=total_batch/n_training
...
gep = GEP(args.num_bases, batch_num, args.clip0, args.clip1, args.power_iter, batch_select).cuda(device)
## attach auxiliary data to GEP instance
gep.public_inputs = public_inputs
gep.public_targets = public_targets

gep2 = GEP(args.num_bases, batch_num, args.clip0, args.clip1, args.power_iter, batch_select).cuda(device)
## attach auxiliary data to GEP instance
    ...
    for batch_idx in range(steps):
        ...
        if (args.private):
            ...
            for num in range(int(batch_num)):
                true_inputs = inputs[num * batch_select : (num + 1) * batch_select]
                true_targets = targets[num * batch_select : (num + 1) * batch_select]
                temp_optimizer.zero_grad()
                true_outputs = temp_net(true_inputs)
            ...
            temp_optimizer.zero_grad()
            temp_net.gep.get_anchor_space2(temp_net, temp_optimizer, loss_func=loss_func, logging=logging)

            batch_grad_list = []
            for name, p in temp_net.named_parameters():
                batch_grad_list.append(grad_list[name].reshape(grad_list[name].shape[0], -1))
        ...
        item_net_dict = copy.deepcopy(temp_net.state_dict())
        tau = lr_temp * gap_rate
        model_mix(temp_net, temp_net2, tau)
        temp_optimizer.step()
        temp_net2.load_state_dict(item_net_dict)
