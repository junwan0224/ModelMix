...
def clip_column(tsr, clip=1.0, inplace=True):
    if (inplace):
        inplace_clipping(tsr, torch.tensor(clip).cuda(tsr.device))
    else:
        norms = torch.norm(tsr, dim=1)
        if use_trunc:
            size = norms.shape[0]
            for i in range(size):
                trunc_val = trunc_ratio * max(norms[i], clip)
                tsr[i] = torch.clamp(tsr[i], min=-trunc_val, max=trunc_val)
            norms = torch.norm(tsr, dim=1)
...
class GEP(nn.Module):
    ...
    def __init__(self, num_bases, batch_size, clip0=1, clip1=1, power_iter=1, batch_select=1):
