...
parser.add_argument('--lr', default=0.15, type=float, help='base learning rate (default=0.1)')
...
parser.add_argument('--eps', default=600, type=float, help='privacy parameter epsilon')



#parser.add_argument('--eps', default=41, type=float, help='privacy parameter epsilon')
#parser.add_argument('--eps', default=54.7795, type=float, help='privacy parameter epsilon')
#parser.add_argument('--eps', default=42.74, type=float, help='privacy parameter epsilon')
...
if(args.dataset == 'svhn'):  ## For SVHN, we concatenate training samples and extra samples to build the training set.
    train_set, trainloader, extraloader, testloader, n_training, n_test = get_data_loader('svhn', batchsize = args.batchsize)
    ...
    train_set, trainloader, testloader, n_training, n_test = get_data_loader('cifar10', batchsize = args.batchsize)
...
def train(epoch):
    ...
    for batch_idx in range(steps):
        ...
        if (args.dataset=='svhn'):
            ...
            if use_iid:
                inputs, targets = iid_sample(train_set, args.batchsize / 50000)
        ...
        if (args.private):
            ...
            temp_net.gep.get_anchor_space(temp_net, loss_func=loss_func, logging=logging)
            ...
            '''
            theta_noise = torch.normal(0, 0.00234, size=clipped_theta.shape,
                                       device=clipped_theta.device)
            grad_noise = torch.normal(0, 0.00234, size=residual_grad.shape,
