...
from models import CNNS, get_num_params, resnet01
from dp_utils import ORDERS, get_privacy_spent, get_renyi_divergence, scatter_normalization
from log import Logger

def main(dataset, augment=False, use_scattering=True, size=None,
         batch_size=8192, mini_batch_size=256, sample_batches=False,
         lr=0.3, optim="SGD", momentum=0.9, nesterov=False,
         noise_multiplier=1, max_grad_norm=1.0, epochs=200,
    ...
    if input_norm == "BN":
        ...
        model2 = CNNS[dataset](K, input_norm="BN", bn_stats=bn_stats, size=size)
    else:
        #model = CNNS[dataset](K, input_norm=input_norm, num_groups=num_groups, size=size)
        #model2 = CNNS[dataset](K, input_norm=input_norm, num_groups=num_groups, size=size)
        model = resnet01()
        model2 = resnet01()
    ...
    if use_scattering and augment:
            ...
            model2 = nn.Sequential(copy.deepcopy(scattering), model2)
            train_loader = torch.utils.data.DataLoader(
    ...
    for epoch in range(0, epochs):
        ...
        if noise_multiplier > 0:
            ...
            if max_epsilon is not None and epsilon >= max_epsilon:
                print("epsilon: ", eps_list)
                print("test accuracy: ", test_list)
                return
        else:
            epsilon = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_norm', default="BN", choices=["GroupNorm", "BN"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_norm', default="GroupNorm", choices=["GroupNorm", "BN"])
    parser.add_argument('--num_groups', type=int, default=27)
