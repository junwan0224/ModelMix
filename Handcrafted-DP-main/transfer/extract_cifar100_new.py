...
# extract the mean and variance of a cifar100 dataset's feature
# load the model from model_best file and compute everything except the last identification layer, this basically computes the feature of the samples
# compute the mean and variance of the features and save it
...
checkpoint = torch.load("model1.th")
...
with torch.no_grad():
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        f = model(inputs.cuda())
        if not setup:
            matrix = torch.randn([f.shape[-1], f.shape[-1]])
            setup = True
        if mul_matrix:
            features_cifar100_train.append(torch.matmul(f, matrix.cuda()).cpu().numpy())
        else:
            features_cifar100_train.append(f.cpu().numpy())
...
with torch.no_grad():
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        f = model(inputs.cuda())
        if mul_matrix:
            features_train.append(torch.matmul(f, matrix.cuda()).cpu().numpy())
        else:
            features_train.append(f.cpu().numpy())
...
with torch.no_grad():
    for batch_idx, (inputs, targets) in enumerate(testloader):
        f = model(inputs.cuda())
        if mul_matrix:
            features_test.append(torch.matmul(f, matrix.cuda()).cpu().numpy())
        else:
            features_test.append(f.cpu().numpy())
