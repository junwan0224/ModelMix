...
def main():
    ...
    model2 = torch.nn.DataParallel(resnet.__dict__[args.arch]())
    model3 = torch.nn.DataParallel(resnet.__dict__[args.arch]())
    model.cuda(device)
    model2.cuda(device)
    model3.cuda(device)
    ...
    optimizer2 = torch.optim.SGD(model2.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    optimizer3 = torch.optim.SGD(model3.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                        milestones=[100, 150], last_epoch=args.start_epoch - 1)
    lr_scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer2,
                                                        milestones=[100, 150], last_epoch=args.start_epoch - 1)
    lr_scheduler3 = torch.optim.lr_scheduler.MultiStepLR(optimizer3,
                                                        milestones=[100, 150], last_epoch=args.start_epoch - 1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for j in temp_dict.keys():
            ...
            alpha1 = torch.div(torch.ones(dictShape),2).cuda(device)
            temp_dict[j] = alpha1 * temp_dict[j] + (torch.ones(dictShape).cuda(device) - alpha1) * temp_dict2[j]
        model3.load_state_dict(temp_dict)
        train_one(train_loader, model3, criterion, optimizer3, epoch)
        lr_scheduler3.step()
        ...
        prec2 = validate(val_loader, model2, criterion)
        print("result of mixed model:")
        prec3 = validate(val_loader, model3, criterion)
...
def train_one(train_loader, model, criterion, optimizer, epoch):
    """
        Run one train epoch
    """
    batch_time = AverageMeter()
