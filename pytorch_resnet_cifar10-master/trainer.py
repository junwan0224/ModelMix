...
def main():
    ...
    model = torch.nn.DataParallel(resnet.__dict__[args.arch](), device_ids = [1, 0])
    model.cuda(device)
    model2 = torch.nn.DataParallel(resnet.__dict__[args.arch](), device_ids = [1, 0])
    model2.cuda(device)
