...
class PerSampleGradientClipper:
    ...
    def clip_and_accumulate(self) -> None:
        ...
        val = torch.mean(all_norms[0]).item() * 2
        self.norm_clipper.set_thresholds(val)
        
        #print("mean: ", torch.mean(abs(all_norms[0])), " 90%:", numpy.percentile(abs(all_norms[0].cpu().numpy()), 90))
    ...
    def _mask_grad_sample (self, name, param, step):
        if 'conv' in name:
            param.grad_sample = param.grad_sample * self.mask[step].unsqueeze(0)
            '''
            batch_size = param.grad_sample.shape[0]
            for i in range(batch_size):
                param.grad_sample[i] = param.grad_sample[i] * self.mask[step]
            '''
    ...
    def _named_grad_samples2(self) -> Iterator[Tuple[str, torch.Tensor]]:
        r"""
        Helper function to get names and per-sample gradients for parameters
        that required grad.
        Returns:
        ...
        for n, p in self.module.named_parameters():
            if "conv" in n and p.requires_grad:
                output.append((n, self._mask_grad_sample(n, p, step)))
                step = step + 1
            elif p.requires_grad:
                output.append((n, p.grad_sample))
        return tuple(output)
