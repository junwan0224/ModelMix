...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet01',
                    choices=model_names,
...
def main():
    ...
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,milestones=[150, 200, 250], last_epoch=args.start_epoch - 1)
...
def train(train_loader, model, model2, criterion, optimizer, optimizer2, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for name, param in temp_model.named_parameters():
            norm_val_0 += torch.norm(param.grad)*torch.norm(param.grad)

        norm_val_0 = torch.sqrt(norm_val_0)


        #norm_val = torch.nn.utils.clip_grad_norm_(temp_model.parameters(), max_norm=clip_norm, norm_type=clip_type)

        total_norm += norm_val_0
        norm_times += 1

        if clip_method == 1:
            #if norm_val.item() >= clip_norm:
                #scale = clip_norm
            if norm_val_0 > clip_norm:
                scale = clip_norm/norm_val_0
            else:
                #scale = norm_val.item()
                scale = 1
        ...
        if (i+1) % true_batch == 0:
            ...
            for name, param in temp_model.named_parameters():
                if clip_type == 2.0:
                   ...
                   #agg_grad = agg_grad+torch.norm(torch.div(sum_grad[name], true_batch))*torch.norm(torch.div(sum_grad[name], true_batch))
                if clip_type == 1.0:
                   ...
                   #agg_grad = agg_grad+torch.norm(torch.div(sum_grad[name], true_batch))*torch.norm(torch.div(sum_grad[name], true_batch))
            temp_optimizer.step()
            #print(torch.sqrt(agg_grad))
            #norm_val2 = torch.nn.utils.clip_grad_norm_(temp_model.parameters(), max_norm=math.inf, norm_type=clip_type)
            #print("norm value 2", norm_val2, "norm value 1", norm_val)
