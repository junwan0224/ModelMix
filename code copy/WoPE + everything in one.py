...
use_trunc = False
use_norm = True
use_prune_after_norm = True
use_mix = True
seperate_clip = False
use_bn_ratio = False
use_public_mask = True
...
print("Pruning:",prune_percentage,prune_percentage_2,"Gap",gap_rate,"CLIP NORM",clip_norm,"Grouping",batch_select,"True Batch", true_batch, "Noise",noise_scale, "Num of Epochs", num_epoch, "Device", dev)
...
def main():
    ...
    #store_model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=True)
    ...
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40, 60, 80],
    #                                                    last_epoch=args.start_epoch - 1)
    lr_scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer2, milestones=[50, 75],
                                                         last_epoch=args.start_epoch - 1)
    lr_scheduler3 = torch.optim.lr_scheduler.MultiStepLR(optimizer3, milestones=[50, 75],
                                                         last_epoch=args.start_epoch - 1)

    lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.988, last_epoch=args.start_epoch - 1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, model2, store_model, criterion, optimizer, optimizer2, optimizer3, epoch)
        lr_scheduler.step()
...
def train(train_loader, model, model2, store_model, criterion, optimizer, optimizer2, optimizer3, epoch):
    """
        Run one train epoch
    """
