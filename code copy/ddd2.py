...
clip_type = 2.0
clip_norm = 1000000
fc_clip_norm = clip_norm
true_batch = 1

if use_norm:
    print("Use Norm", use_norm, "CLIP NORM", clip_norm, "Batchnorm", keep_bn, "Use Pruning", use_prune,
          prune_percentage_0, "Use Mix", use_mix, gap_0, "Noise", noise_mul, "Num of Epochs", num_epoch, "Device", dev)

if use_trunc:
    print("Use Trunc", use_trunc, "Max Value", max_val, "Batchnorm", keep_bn, "Use Pruning", use_prune,
          prune_percentage_0, "Use Mix", use_mix, gap_0,
...
def main():
    ...
    lr_scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer2, milestones=[30, 50],
                                                         last_epoch=args.start_epoch - 1)
    lr_scheduler3 = torch.optim.lr_scheduler.MultiStepLR(optimizer3, milestones=[30, 50],
                                                         last_epoch=args.start_epoch - 1)
...
def train(train_loader, model, model2, store_model, criterion,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if print_detail_norm and i > 0 and i % (args.print_freq * 2) == 0:
            sum_norms = calc_persample_norm(temp_model)
            average_norm = sum_norm.sum() / batch_select
        ...
        if erase_large:
            sum_norms = calc_persample_norm(temp_model)
            #threshold_norm = erase_threshold * sum_norms.sum() / batch_select
            threshold_norm = erase_threshold * sum_norms.median()
            sample_used = torch.where(sum_norms < threshold_norm, 0.0, 1.0)
            for name, param in temp_model.named_parameters():
                if "bn" in name:
                    continue
                for j in range(batch_select):
                    if sample_used[j] == torch.tensor(0.0):
                        param.grad_sample[j] = torch.zeros(param.grad_sample[j].shape)
            temp_batch_num = sample_used.sum()
        else:
            sample_used = torch.ones(batch_select)
            temp_batch_num = batch_select
        '''
        sample_used = torch.ones(batch_select)
        temp_batch_num = batch_select
        ...
        if keep_bn:
            ...
            if erase_large:
                new_input = torch.empty(0).cuda(device)
                new_target = torch.empty(0).cuda(device)
                for j in range(batch_select):
                    if sample_used[j] != torch.tensor(0.0):
                        new_input = torch.cat((new_input, input_var[j].unsqueeze(0)), 0)
                        new_target = torch.cat((new_target, target_var[j].unsqueeze(0)), 0)
            ...
            store_loss = criterion(store_output, new_target.long())
            store_model.zero_grad()
            store_loss.backward()

            for (temp_pair, store_pair) in zip(temp_model.named_parameters(), store_model.named_parameters()):
                name, param = temp_pair
                name2, param2 = store_pair
                assert (name == name2)
                if "bn" in name:
                    param.require_grad = True
                    new_shape = [batch_select] + list(param2.grad.shape)
                    grad_sample_list[name] = param2.grad.unsqueeze(0).expand(new_shape)
                else:
                    grad_sample_list[name] = param.grad_sample

            for name, param in store_model.named_parameters():
                true_grad[name] = param.grad
        else:
            for name, param in temp_model.named_parameters():
                grad_sample_list[name] = param.grad_sample
            for name in grad_sample_list:
                true_grad[name] = torch.squeeze(torch.sum(grad_sample_list[name], 0))
        ...
        if use_prune:
            ...
            for name in grad_sample_list:
                ...
                noise1 = laplace_dist.sample(grad_sample_list[name][0].shape).cuda(device) * percent_noise_scale
                # we use mask to temporarily store the sum of the gradient, it will be switched to mask later
                mask[name] = grad_sample_list[name].sum(0).squeeze() / temp_batch_num + noise1
            ...
            percentile_value = torch.tensor(percentile_value).cuda(device)

            for name, param in temp_model.named_parameters():
                if "bn" in name or "linear" in name:
                    mask[name] = torch.ones(grad_sample_list[name][0].shape).cuda(device)
                mask[name] = torch.where(mask[name] < percentile_value,
                                         torch.tensor(0.0).cuda(device), torch.tensor(1.0).cuda(device))
                large_mask = mask[name].unsqueeze(0).expand(grad_sample_list[name].shape)
                grad_sample_list[name] = grad_sample_list[name] * large_mask
        else:
            for name in grad_sample_list:
                mask[name] = torch.ones(grad_sample_list[name][0].shape).cuda(device)
        ...
        if use_trunc:
            for name in grad_sample_list:
                grad_sample_list[name] = torch.clamp(grad_sample_list[name], min=-max_val, max=max_val)
        ...
        if use_mix:
            ...
            for name, param in temp_model.named_parameters():
                ...
                if torch.min(gap_opr) < tau:
                    ...
                    sign_opr = torch.sign(temp_dict[name] - temp_dict2[name]) * mask[name]
        ...
        if not keep_bn and use_PEStep:
            ...
            if use_norm:
                ...
                for name, param in temp_model.named_parameters():
                    ...
                    param.grad = torch.einsum("i,i...", clip_factor, grad_sample_list[name]) / temp_batch_num
            else:
                for name, param in temp_model.named_parameters():
                    param.grad = grad_sample_list[name].sum(0).squeeze() / temp_batch_num
