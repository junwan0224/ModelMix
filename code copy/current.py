...
parser.add_argument('--resume', default='save_temp/model0.th', type=str, metavar='PATH', help ='path to latest checkpoint (default: none)')
...
def main():
    ...
    #lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 80, 110], last_epoch=args.start_epoch - 1)
    ...
    lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99, last_epoch=args.start_epoch - 1)

    lr_scheduler2 = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99, last_epoch=args.start_epoch - 1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        '''
        train(use_public_expand, train_loader, model, model2, store_model, criterion,
        ...
        if epoch > 0 and epoch % args.save_every == 0:
            ...
            }, is_best, filename=os.path.join(args.save_dir, 'checkpoint1.th'))

        save_checkpoint({
            'state_dict': model.state_dict(),
            'best_prec1': best_prec1,
        }, is_best, filename=os.path.join(args.save_dir, 'model1.th'))
...
def mixup_public_data(private_x, private_y, public_x, public_y, index, index0, alpha = 0.1):
    ...
    mixed_x = (1 - lam) * x1 + lam * x2 + 0 * x3
    return mixed_x, y1, y2, y3, lam

def mixup_criterion(criterion, pred, y_a, y_b, y_c, lam):
    return (1 - lam) * criterion(pred, y_a) + lam * criterion(pred, y_b) + 0*criterion(pred, y_c)
...
def train(use_public_expand, train_loader, model, model2, store_model, criterion,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        '''
        index = torch.randint(1, 10000, (public_batch_size,)).cuda(device)
        index0 = torch.randint(1, 1000, (public_batch_size,)).cuda(device)
