...
expand_batch_size = 200
calc_bn_size = 50
true_batch = 2400 / batch_select
prune_percentage = 0
...
def get_SVHN_data ():
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.SVHN(root='./data', split='extra', transform=transforms.Compose([
    ...
    public_input = torch.empty(0).cuda(device)
    for i, (input, target) in enumerate(train_loader):
        public_input = torch.cat((public_input, input[target == 0].cuda(device)), 0)
    return public_input
...
def main():
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50, 80, 110],
                                                        last_epoch=args.start_epoch - 1)
    lr_scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer2, milestones=[50, 80, 110],
                                                         last_epoch=args.start_epoch - 1)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model, model2, store_model, criterion,
            optimizer, optimizer2, optimizer3, public_input, epoch)
        lr_scheduler.step()
        lr_scheduler2.step()
...
def mixup_data(x, y, alpha = 0.1):
    ...
    y_a, y_b = y, y[index]
    return mixed_x, y_b, y_a, lam

def mixup_public_data(private_x, private_y, public_x, public_y, alpha = 0.1):
    if alpha > 0:
    ...
    mixed_x = (1 - lam) * x1 + lam * x2
    return mixed_x, y1, y2, lam

def mixup_SVHN_data(private_x, private_y, x2, y2, alpha = 0.1):
    if alpha > 0:
        lam = np.random.beta(alpha, alpha) * 0.1
    else:
        lam = 1

    repeat_num = int(public_batch_size / private_x.size()[0])
    x1, y1 = private_x.repeat(repeat_num, 1, 1, 1), private_y.repeat(repeat_num)
    mixed_x = (1 - lam) * x1 + lam * x2
    return mixed_x, y1, y2, lam
...
def train(train_loader, model, model2, store_model, criterion,
    ...
    index = torch.randint(batch_size, (public_batch_size,)).cuda(device)
    x2, y2 = public_input[index], torch.ones(public_batch_size).long().cuda(device) * 10

    for i, (input, target) in enumerate(train_loader):
        if input.size()[0] < batch_select:
            break
        ...
        if use_expand:
            ...
            for j in range(times):
                ...
                if j == 0:
                    loss = mixup_criterion(criterion, outputs, targets_a, targets_b, lam) / times
                else:
                    loss += mixup_criterion(criterion, outputs, targets_a, targets_b, lam) / times
        elif use_public_expand:
            inputs, y1, y2, lam = mixup_SVHN_data(input_var, target_var, x2, y2)
            outputs = temp_model(inputs)
            loss = mixup_criterion(criterion, outputs, y1, y2, lam)
        else:
        ...
        if (i + 1) % true_batch == 0:
            ...
            if use_prune_after_norm:
                ...
                percentile_value = noise_scale / 5
